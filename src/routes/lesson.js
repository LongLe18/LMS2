...
router.get('/:id/edit', authToken, authRole([2], 6), tryCatch(lessonController.getUpdate));
router.put(
    '/:id', authToken, authRole([2], 6), 
    upload.single('link_bai_giang'),
    tryCatch(lessonController.putUpdate)
);
router.delete('/:id', authToken, authRole([2], 6), tryCatch(lessonController.deleteById));
router.delete('/:id/force', authToken, authRole([2], 6), tryCatch(lessonController.forceDelete));
router.patch('/:id/restore', authToken, authRole([2], 6), tryCatch(lessonController.restore));
//router.get('/all', authToken, tryCatch(lessonController.getAllv2));
router.get('/filter', authToken, authRole([2]),tryCatch(lessonController.getByFilter));
router.get('/by_thematic/:id', authToken, tryCatch(lessonController.getByIDThematic));
