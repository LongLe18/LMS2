...
router.get('/:id/edit', authToken, authRole([2], 6), tryCatch(examController.getUpdate));
router.put(
    '/:id', authToken, authRole([2], 6), 
    upload.single('anh_dai_dien'),
    tryCatch(examController.putUpdate)
);
router.delete('/:id/force', authToken, authRole([2], 6), tryCatch(examController.forceDelete));
router.delete('/clear', authToken, authRole([2], 6), tryCatch(examController.clearAll));
router.get('/state_change/:id', authToken, authRole([2], 6), tryCatch(examController.stateChange));
router.get('/:id/publish', authToken, authRole([2], 6), tryCatch(examController.publish));
...
router.get('/reuse/:id', authToken, authRole([2], 6), tryCatch(examController.reuse));
