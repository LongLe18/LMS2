...
router.get('/:id/edit', authToken, authRole([2], 2), tryCatch(staffController.getUpdate));
router.put(
    '/:id', authToken, authRole([2], 2),
    upload.single('anh_dai_dien'),
    tryCatch(staffController.putUpdate)
);
router.get('/change-state/:id', authToken, authRole([2], 2), tryCatch(staffController.stateChange));
router.delete('/:id/force', authToken, authRole([2], 2), tryCatch(staffController.forceDelete));
