...
const ForgotPasswordPage = (props) => {
    ...
    const onSubmit = (values) => {
        ...
        dispatch(userActions.forgotPassword({ typeUser: typeUser, email: data }, callback));
    };
                        ...
                        <Form className="login-form" form={form} onFinish={onSubmit}>
                            ...
                            <Form.Item>
                                ...
                                <div className="other-links">
                                    {' '}
                                    <button className="login-form-forgot font-weight-5" 
                                        onClick={() => {
                                            if (typeUser === '1') {
                                                props.history.push("/auth/hocvien")
                                            }
                                            if (typeUser === '2') {
                                                props.history.push("/auth/giaovien")
                                            }
                                            if (typeUser === '3') {
                                                props.history.push("/auth/nhanvien")
                                            }
                                        }}
                                    >
                                        Đăng nhập
