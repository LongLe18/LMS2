...
const ExamDetail = () => {
    ...
    }, [params.idmodule, params.idthematic]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const onChangeAnswerText = (value, question) => {
        if (isDoing && localStorage.getItem('question')) {
    ...
    const onSaveHistory = () => {
        ...
        const info = {
            "thoi_gian_lam_bai": secondsToMinutes(timePassedInSecond),
            "thoi_diem_ket_thuc": moment().toISOString()
        }
    ...
    //     const callback = (res) => {
    //         if (res.status === 200 && res.statusText === 'OK') {
    //             window.location.href = `/luyen-tap/lam-kiem-tra/${params.idmodule}/${moment().toNow()}/${params.idthematic}/${res.data.data.de_thi_id}/${res.data.data.dthv_id}/${params.idCourse}`;
    //         }
    //     };
    ...
    const saveComment = (cau_hoi_id, mo_dun_id, chuyen_de_id) => {
            ...
            if (res.data.status === 'success' && res.statusText === 'OK') {
                ...
                if ((!state.isEdit && state.isReplied) || (!state.isEdit && !state.isReplied) ) { // Chỉ tạo thông báo khi thêm hoặc phản hồi
                    const note = {
                        ...
                        "link_lien_ket": `${chuyen_de_id}/${params.idmodule}/${hashids.decode(params.idCourse)}/${params.id}/${params.idExamUser}/${res.data.data.binh_luan_id}`
                    };
        ...
        if (!state.isReplied && !state.isEdit) {
            ...
            formData.append('mo_dun_id', params.idmodule);
            formData.append('loai_hoi_dap', 1)
            formData.append('lien_ket_id', `${params.id}/${cau_hoi_id}/${chuyen_de_id}/${params.idExamUser}/1`);
            if (state.fileImg !== '') {
