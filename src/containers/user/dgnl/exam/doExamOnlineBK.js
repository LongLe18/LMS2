...
import { getAnswerCols } from 'helpers/common.helper';
...
export default function ExamOnlineDetaiDGTD() {
    ...
    const handleDragEnd = (result) => {
        ...
        if (destination.droppableId.startsWith("gap")) {
            ...
            const word = questionData.find((w) => (w.cau_hoi_id + index).toString() === result.draggableId);
            if (!word) return
    ...
    const questionDragAndDrop = (question, key) => {
        return (
            <DragDropContext onDragEnd={handleDragEnd} key={key}>
                <Card title={`Câu ${currentQuestion + 1}`}
                    extra={
    ...
    const renderChoiceQuestion = (question, key) => {
        return (
            <Card title={`Câu ${currentQuestion + 1}`} key={key}
                extra={
                    <Tooltip title="Đánh dấu câu hỏi">
                        <Button shape="circle" icon={<BookOutlined />} onClick={() => handleMarkQuestion()}/>
                    </Tooltip>
                    ...
                    {questionData.map((question, index) => {
                        if (index === currentQuestion) {
                            if (question?.cau_hoi?.loai_cau_hoi === 3) // kéo thả
                                return questionDragAndDrop(question, index);
                                ...
                                return renderRightWrongQuestion(question, index);
