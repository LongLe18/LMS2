...
const ExamDetailPage = () => {
        ...
        const callback = (res) => {
            ...
            if (res.status === 'success') {
                if (res.data.loai_de_thi_id === 1) { // loại chuyên đề
                    setState({...state, showThematic: false, showCourse: false, showModule: false});
    ...
    const renderExamCategories = () => {
        ...
        return (
          <Select
            ...
            onChange={(typeId) => {
                if (typeId === 1) {setState({...state, showThematic: false, showCourse: false, showModule: false})}
                else if (typeId === 2) {setState({...state, showThematic: true, showCourse: false, showModule: false})}
                else {setState({...state, showThematic: true, showCourse: false, showModule: false})}
            }}
          >
            {options}
          </Select>
    ...
    const renderThematic2 = () => {
        ...
        return (
          <Select
            disabled={(exam.data.loai_de_thi_id === 1 ) ? true : false}
            showSearch={false}
    ...
    const handleSaveExam = (values) => {
        ...
        if (values.loai_de_thi_id === 1) {
            ...
            if (values.chuyen_de_id === "") {
                notification.error({
                    message: 'Thông báo',
                    description: 'Bạn chưa chọn chuyên đề',
                })
                return;
            }
