...
const ReplyPage = (props) => {
    ...
    if (comments.status === 'success') {
        state.comment = comments.data.map((comment, index) => ({...comment, key: index}))
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (binh_luan_id) => (
                <Space size="middle">
                    <Button  type="button"  onClick={() => showModal(binh_luan_id)} className="ant-btn ant-btn-round ant-btn-primary">Chuyển xử lý</Button>
                    <Button shape="round" onClick={() => detailComment(binh_luan_id)} type="primary" >Chi tiết</Button> 
                    <Button shape="round" onClick={() => deleteComment(binh_luan_id)} type="danger" >Xóa</Button> 
                </Space>
            ),
        },
    ];
    ...
    // Handle modal forward 
    const renderModalForward = () => {
        return (
            <>
                <h2 className="form-title">Chuyển tiếp câu hỏi</h2>
                <Form form={formModal} className="login-form app-form" name="login-form" onFinish={handleForward} labelCol={{span:7}}>
                    <Form.Item
                        label="Chuyên ngành"
                        className="input-col"
                        name="chuyen_nganh"
                    ...
                    <Form.Item className="button-col" style={{marginBottom: 0}}>
                        <Button shape="round" type="primary" htmlType="submit" >Chuyển tiếp</Button>
                    </Form.Item>
                </Form>
            </>
        )
    ...
    const showModal = (binh_luan_id) => {
        setState({ ...state, idComment: binh_luan_id });
        setIsModalVisible(true);
        dispatch(commentAction.getCOMMENT({ id: binh_luan_id })); // Lấy bình luận theo id
    };
    ...
    const handleForward = (values) => {
        const callback = (res) => {
            if (res.status === 200 && res.statusText === 'OK') {
                const note = {
                    "loai_thong_bao": 0,
                    "lien_ket_id": state.idComment,
                    //// bổ sung link_lien_ket
                    "link_lien_ket": state.idComment
                };
                dispatch(notificationAction.CreateNOTIFICATION({ formData: note, 
                    idModule: comment.data.mo_dun_id !== null ? comment.data.mo_dun_id : '', 
                    type: comment.data.loai_hoi_dap, idCourse: comment.data.khoa_hoc_id,
                    ...
                    idExam: comment.data.loai_hoi_dap === 1 ? comment.data.lien_ket_id.split('/')[0] : '', 
                    index: comment.data.loai_hoi_dap === 1 ? comment.data.lien_ket_id.split('/')[1] : '', 
                    Teacher: values.giao_vien_id, chuyen_tiep: true }, callback2));
            }
        };
        ...
        data.append('phu_trach_id', values.giao_vien_id);
        dispatch(commentAction.EditCOMMENT({ formData: data, idComment: state.idComment }, callback));  
    };
