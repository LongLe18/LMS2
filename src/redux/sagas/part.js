...
function* fetchModulesTeacherByIdCourse(payload) {
    try {
        let endpoint = `${config.API_URL}/modun/by-teacher?khoa_hoc_id=${payload.params.idCourse}&lkh_id=${payload.params.lkh}&trang_thai=${payload.params.status}&search=${payload.params.search}&pageSize=${payload.params.pageSize}&pageIndex=${payload.params.pageIndex}`;
        const response = yield call(getApiAuth, endpoint);
        const result = yield response.data;
        yield put({ type: actions.part.GET_MODULES_TEACHER_SUCCESS, result: result });
        if (payload.callback) {
            payload.callback(result);
        }
    } catch (error) {
        yield put({ type: actions.part.GET_MODULES_TEACHER_FAILED, error: error });
        let messageError = error.response.status === 403 ? error.response.data : '';
        notification.error({
            message: get(error, 'response.data.error', 'Tải dữ liệu phần học đã thất bại ' + messageError),
        });
...
function* fetchFilter(payload) {
    try {
        ...
        if (payload.params.pageIndex && payload.params.pageSize) {
            endpoint += `&pageIndex=${payload.params.pageIndex}&pageSize=${payload.params.pageSize}`;
        }
...
export function* loadModulesTeacherByIdCourse() {
    yield takeEvery(actions.part.GET_MODULES_TEACHER, fetchModulesTeacherByIdCourse);
}
