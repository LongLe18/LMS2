...
const findAll = async (req, res) => {
    ...
    for (const menu of menus) {
        ...
        if (menu.loai_menu_id === 2) {
            const ids = menu.gia_tri
                ?.split(',')
                .map((id) => parseInt(id))
                .filter(Boolean);
            if (ids && ids.length > 0) {
                const program = await sequelize.query(
                    `SELECT kct_id AS \`key\`, ten_khung_ct AS \`label\` FROM khung_chuong_trinh WHERE kct_id IN (:ids)`,
                    {
                        replacements: { ids },
        ...
        if (menu.loai_menu_id === 3) {
            ...
            if (ids && ids.length > 0) {
                const program = await sequelize.query(
                    `SELECT loai_kct FROM khung_chuong_trinh WHERE kct_id IN (:ids)`,
                    {
                        replacements: { ids },
                        type: sequelize.QueryTypes.SELECT,
                    }
