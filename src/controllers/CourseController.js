...
const addStudent = async (req, res) => {
    let data=[];
    let hoc_vien_ids=req.body.hoc_vien_id.split(',');
    for(const hoc_vien_id of hoc_vien_ids){
        data.push({
        ...
        for(const course of courses){
            ...
            for(var index=15; index>=0; index--){
                thematic=await sequelize.query(`
                    ...
                    AND de_thi.khoa_hoc_id=${course.khoa_hoc_id} AND de_thi_hoc_vien.dat_yeu_cau=true AND 
                    DATEDIFF(NOW(), de_thi_hoc_vien.ngay_tao)<${(index+1)*7} AND DATEDIFF(NOW(), de_thi_hoc_vien.ngay_tao)>=${index*7}`,
                    {type: sequelize.QueryTypes.SELECT});
                thematics.push(thematic[0]);
            }
            data.push({
